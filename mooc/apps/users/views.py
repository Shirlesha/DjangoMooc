...
from schools.models import SchoolName, Lecturer
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            user_message.message = "欢迎注册Shirlesha--MOOC爱上学习!! --系统自动消息"
            user_message.save()
            # 发送注册激活邮件
...
#         # 上面的fav_orgs只是存放了id。我们还需要通过id找到学校对象
#         for fav_org in fav_orgs:
#             # 取出fav_id也就是学校的id。
#             org_id = fav_org.fav_id
#             # 获取这个学校对象
#             org = SchoolName.objects.get(id=org_id)
#             org_list.append(org)
...
class MyFavLecturerView(LoginRequiredMixin, View):
    """我收藏的授课讲师"""
    login_url = '/login/'
    redirect_field_name = 'next'
    ...
    def get(self, request):
        lecturer_list = []
        fav_lecturers = UserFavorite.objects.filter(
            user=request.user, fav_type=3)
        # 上面的fav_orgs只是存放了id。我们还需要通过id找到学校对象
        for fav_lecturer in fav_lecturers:
            # 取出fav_id也就是学校的id。
            lecturer_id = fav_lecturer.fav_id
            # 获取这个学校对象
            lecturer = Lecturer.objects.get(id=lecturer_id)
            lecturer_list.append(lecturer)
        return render(request, "usercenter-fav-lecturer.html", {
...
class MyMessageView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        try:
            ...
            page = 1
        # 这里指从allSchoolName中取五个出来，每页显示12个
        p = Paginator(all_message, 12)
...
class IndexView(View):
    ...
    def get(self, request):
        ...
        # 所属学校
        school_name = SchoolName.objects.all()[:15]
        return render(request, 'index.html', {
